...
tf.flags.DEFINE_string('train_dir', './tmp/fruits_train',
                           """Directory where to write event logs """
                           """and checkpoint.""")
tf.flags.DEFINE_integer('max_steps', 1,
...
def train():
  ...
  with tf.Graph().as_default():
    ...
    with tf.device('/cpu:0'):
      images, labels = fruits_net.inputs()
    ...
    # Calculate loss.
    loss = fruits_net.loss(logits, labels)

    # Build a Graph that trains the model with one batch of examples and
    # updates the model parameters.
    train_op = fruits_net.train(loss, global_step)
