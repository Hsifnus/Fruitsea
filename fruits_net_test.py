...
tf.flags.DEFINE_string('eval_dir', './tmp/logs',
                           """Directory where to write event logs.""")
tf.flags.DEFINE_string('eval_data', './fruits-360/Test',
                           """Either 'test' or 'train_eval'.""")
tf.flags.DEFINE_string('checkpoint_dir', './tmp/fruits_train',
                           """Directory where to read model checkpoints.""")
tf.flags.DEFINE_integer('eval_interval_secs', 5,
                            """How often to run the eval.""")
tf.flags.DEFINE_integer('num_examples', 30,
                            """Number of examples to run.""")
tf.flags.DEFINE_boolean('run_once', True,
                         """Whether to run eval only once.""")
tf.flags.DEFINE_integer('num_batches', int(fruits_net.NUM_EXAMPLES_PER_EPOCH_FOR_EVAL / FLAGS.batch_size),
...
def eval_once(saver, summary_writer, top_k_op, summary_op):
  ...
  with tf.Session() as sess:
    ...
    try:
      ...
      average_precision += precision / FLAGS.num_batches
...
def evaluate():
  ...
  with tf.Graph().as_default() as g:
    ...
    # inference model.
    logits = fruits_net.inference(images)
    ...
    variable_averages = tf.train.ExponentialMovingAverage(
        fruits_net.MOVING_AVERAGE_DECAY)
...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  print("Running %d batches..." % (FLAGS.num_batches))
