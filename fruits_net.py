...
NUM_EXAMPLES_PER_EPOCH_FOR_EVAL = 12709
...
SAMPLING_CHANCE = 1          # Chance that a sampled image will be included into a batch.
...
tf.flags.DEFINE_string('data_dir', './fruits-360/',
...
def inputs(eval_data=False):
  ...
  Args:
    eval_data: Boolean flag. Whether we are handling test data or not.
  Returns:
  ...
  np_images, np_labels = [], []
  while len(np_images) < FLAGS.batch_size:
    try:
        path, label = next(test_files_list if eval_data else training_files_list)
    except StopIteration:
        if eval_data:
            test_files_list = net_utils.fetch_next_file_and_label(os.path.join(FLAGS.data_dir, 'Test'))
        else:
            training_files_list = net_utils.fetch_next_file_and_label(os.path.join(FLAGS.data_dir, 'Training'))
        if len(np_images) >= FLAGS.batch_size:
            break
        else:
            continue
  ...
  images, labels = tf.convert_to_tensor(np_images), tf.convert_to_tensor(np_labels)
  if FLAGS.use_fp16:
...
def inference(images):
  ...
  softmax_linear = tf.Print(softmax_linear, [tf.argmax(softmax_linear[i]) for i in range(FLAGS.batch_size)])
...
def loss(logits, labels):
  ...
  # logits = tf.Print(logits, [logits[i] for i in range(FLAGS.batch_size)])
...
def set_sampling_chance(new_chance):
    global SAMPLING_CHANCE
    SAMPLING_CHANCE = new_chance
